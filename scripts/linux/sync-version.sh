...
read_lockfile_version() {
  local lockfile="${1:?}"
  perl -ne '
    if (/^name = "orbital_movement_gdextension"$/) { $want = 1 }
...
sync_lockfile_versions() {
  ...
  local version="${2:?}"
  perl -i -pe '
    if (/^name = "(?:orbital_movement_gdextension|orbital_movement_gdextension_gd|orbital_movement_gdextension_cli|orbital_movement_gdextension_visualizer)"$/) { $want = 1 }
    elsif ($want && /^version = "/) {
...
sync_version_repo_root() {
  ...
  if [[ -f "${description_md}" ]]; then
    perl -0777 -i -pe \
      's/(name = "orbital_movement_gdextension"\r?\nversion = ")[^"]+(")/${1}'"${version}"'${2}/s' \
      "${description_md}"
  fi
...
verify_version_sync() {
  ...
  if [[ -f "${description_md}" ]]; then
    local doc_version
    doc_version="$(perl -0777 -ne 'if (/name = "orbital_movement_gdextension"\r?\nversion = "([^"]+)"/s) { print $1; exit }' "${description_md}")"
  ...
  if [[ -f "${cargo_lock}" ]]; then
    ...
    if [[ "${lock_version}" != "${version}" ]]; then
      echo "error: VERSION (${version}) does not match Cargo.lock orbital_movement_gdextension version (${lock_version})" >&2
      echo "Run: ./scripts/linux/sync-version.sh" >&2
      return 1
    fi
  fi
